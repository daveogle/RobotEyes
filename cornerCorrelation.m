function [ bLc, bRc ] = cornerCorrelation(image1, image2, N, supportWinSize, searchWinSize, diffType)
	...
	% For the corners in image 1 compare them to the corners in image 2 and find the best match
    for i= 1 : pointsLsize
        pointsInRange = findPointsInRange(pointsL(i,:), pointsR, range);
		cost{i} = Correlation(image1, image2, [pointsL(i,2), pointsL(i,1)], [pointsInRange(:,2),pointsInRange(:,1)], supportWinSize, diffType);
        bestR = cost{i};
        bestL = pointsL(i,:);
